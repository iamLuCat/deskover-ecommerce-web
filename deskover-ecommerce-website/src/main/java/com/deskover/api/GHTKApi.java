...
public class GHTKApi {
	...
	public ResponseEntity<?> doGetFee(@RequestBody FeeGhtk fee,@RequestHeader(value="Token") String Token) throws Exception {
		try {
			...
			if (Objects.requireNonNull(response).getFee() == null) {
				...
				ResponseEntity<String> errorRequest = restTemplate.postForEntity(UrlConstant.GHTK_FEE, request, String.class);
